...
PROJECT_PATH="$ROOT_DIR/VSMineralMasonry.csproj"
...
echo "Building VSMineralMasonry $VERSION"
dotnet build "$PROJECT_PATH" -c Release -p:NuGetAudit=false

if [[ ! -f "$MOD_OUTPUT_DIR/VSMineralMasonry.dll" ]]; then
...
mkdir -p "$DIST_DIR"
ZIP_PATH="$DIST_DIR/vsmineralmasonry-$VERSION.zip"
rm -f "$ZIP_PATH"
